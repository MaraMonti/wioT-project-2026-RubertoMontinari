...
#
# # --- Feature temporali ---
# def crea_feature_temporali(df, ts_col='timestamp_minuto'):
#     df = df.copy()
#     df['hour'] = df[ts_col].dt.hour
#     df['day_of_week'] = df[ts_col].dt.dayofweek
#     df['month'] = df[ts_col].dt.month
#     df['day_of_year'] = df[ts_col].dt.dayofyear
#     df['week_of_year'] = df[ts_col].dt.isocalendar().week.astype(int)
...
#     df['is_weekend'] = df['day_of_week'].isin([5, 6]).astype(int)
#     return df[['Temperature', 'Humidity', 'hour', 'day_of_week', 'month', 'day_of_year', 'week_of_year', 'is_weekend']]
#
# # --- Meteo ---
...
#         start_dt = end_dt - timedelta(days=4)  # 4 giorni: 3 reali + oggi
...
#
#         # --- INTEGRAZIONE: Calcolo consumo giornaliero reale da dati potenza istantanea ---
#         # Calcola delta tempo in ore tra misurazioni successive
...
#         # Energia (Wh) = potenza precedente * delta tempo
...
#
#         # Seleziona ultimi 3 giorni completi
#         giorni_disponibili = df_daily['date'].sort_values().unique()
#         giorni_recenti = giorni_disponibili[-3:]
#
...
#
#         # Scarica meteo per domani
#         domani = datetime.utcnow().date() + timedelta(days=2)
#         meteo_df = get_weather_data(CITY, domani, domani, API_KEY)
#
...
#
#         y_pred_domani = modello.predict(meteo_giorno[feature_cols])[0]
#
#
#         # === Tabella 1: Riepilogo comparativo ===
#         st.subheader("Consumo Giornaliero: Reale vs Predetto")
#
#         # Uso energia_per_giorno per dati reali nel grafico e tabella
#         energia_per_giorno_ultimi3 = energia_per_giorno[energia_per_giorno['data'].isin(giorni_recenti)]
#
#         df_bar = pd.DataFrame({
#             'Data': list(energia_per_giorno_ultimi3['data']) + [domani],
#             'Consumo Reale (kWh)': list(energia_per_giorno_ultimi3['consumo_kWh']) + [None],
#             'Consumo Predetto (kWh)': list(df_recenti['consumo_predetto']) + [y_pred_domani]
#         })
#
#         # Grafico a barre aggiornato con etichette
...
#         # Barre predette
#         bars_pred = ax.bar([i + 0.4 for i in x], df_bar['Consumo Predetto (kWh)'], width=0.4, label='Predetto',
#                            align='center')
#
...
#         st.pyplot(fig)
#
#         # Tabella comparativa: Reale vs Predetto
#         st.subheader("Tabella Consumo Giornaliero: Reale vs Predetto")
#
#         tabella_confronto = df_bar.rename(columns={
#             'Data': 'Data',
#             'Consumo Reale (kWh)': 'Consumo Reale (kWh)',
#             'Consumo Predetto (kWh)': 'Consumo Predetto (kWh)'
#         })
#
#         st.dataframe(tabella_confronto)
#
#         # Mostra valori istantanei (ultimi 3 giorni)
#         oggi = datetime.utcnow().date()
#         tre_giorni_fa = oggi - timedelta(days=3)
#
#         df_consumo_3gg = df_consumo[df_consumo['timestamp'].dt.date >= tre_giorni_fa]
#         df_consumo_3gg = df_consumo_3gg.sort_values(by='timestamp', ascending=False)
#
#         st.subheader("Valori istantanei del consumo (ultimi 3 giorni)")
#         st.write("Questi sono tutti i valori di potenza istantanea (Watt) recuperati dall'API:")
#
#         st.dataframe(df_consumo_3gg[['timestamp', 'value']].rename(columns={
#             'timestamp': 'Timestamp',
#             'value': 'Potenza (W)'
#         }))
#
